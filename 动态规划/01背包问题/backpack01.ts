...
function backpack01(goods: GoodInfo[], capacity: number) {
    const dp = new Array<number[]>(goods.length + 1);
    for (let i = 0; i < goods.length + 1; i++) {
        dp[i] = new Array<number>(capacity + 1).fill(0);
    }
    // console.log(dp);
...
function backpack012(goods: GoodInfo[], capacity: number) {
...
function backpack013(goods: GoodInfo[], capacity: number) {
    const dp = new Array<number>(capacity + 1).fill(0);
